...
class NoteServices(BaseService):
    ...
    async def change_note(
        ...
        async with async_session_maker() as session:
            try:
                result = await session.execute(select(Note).where(Note.id == note_id).options(selectinload(Note.user)))
                ...
                if update_data:
                    await session.execute(update(Note).where(Note.id == note_id).values(update_data))
    ...
    async def update_note_is_done(note_id: int, is_done: bool, current_user: User) -> Note | None:
        async with async_session_maker() as session:
            try:
                result = await session.execute(select(Note).where(Note.id == note_id).options(selectinload(Note.user)))
                note = result.scalar_one_or_none()
                ...
                await session.execute(update(Note).where(Note.id == note_id).values(is_done=is_done))
                await session.commit()
                await session.refresh(note)
    ...
    async def find_by_id_with_access_check(task_id: int, current_user: User) -> Note | None:
        async with async_session_maker() as session:
            if current_user.role == Role.ADMIN:
                ...
                result = await session.execute(select(Note).where(Note.id == task_id, Note.user_id == current_user.id))
